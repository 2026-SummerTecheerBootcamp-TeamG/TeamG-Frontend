      ...
      <div className="border-b border-line bg-[#fcfdfd] px-7 py-5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4 text-[12.5px] text-ink-2">
          <span className="flex items-center gap-1.5">
            ...
            항공 <b className=" text-ink">{formatWon(al.flight)}</b>
          </span>
          <span className="flex items-center gap-1.5">
            <i className="h-2 w-2 rounded-sm bg-teal" />
            숙소 <b className=" text-ink">{formatWon(al.hotel)}</b>
          </span>
          <span className="flex items-center gap-1.5">
            <i className="h-2 w-2 rounded-sm bg-amber" />
            일정 <b className=" text-ink">{formatWon(al.activity)}</b>
          </span>
          <span
            className={`ml-auto text-[12.5px] font-semibold ${al.remaining >= 0 ? "text-teal" : "text-stamp"}`}
          >
            {al.remaining >= 0
          ...
          <div key={i} className={`flex items-center gap-2 ${i > 0 ? "mt-3.5" : ""}`}>
            <div className="flex flex-1 items-center gap-3.5">
              <div className="min-w-[78px]">
                <p className=" text-xs tracking-[0.06em] text-ink-3">{leg.from}</p>
              ...
              <div className="relative h-5 flex-1">
                ...
                <span className="absolute left-1/2 top-0.5 -translate-x-1/2 whitespace-nowrap bg-white px-2 text-[10.5px] text-ink-2">
                  {flight.duration} · {flight.stops === 0 ? "직항" : `경유 ${flight.stops}회`}
                </span>
              </div>
              <div className="min-w-[78px]">
                <p className=" text-xs tracking-[0.06em] text-ink-3">{leg.to}</p>
      ...
      <div className="border-b border-line px-7 py-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1.5 text-[11px] tracking-[0.1em] text-amber">
              {"★".repeat(hotel.stars)}
            ...
            <div className="relative pl-[74px]">
              ...
              {day.items.map((item, j) => (
                <div key={j} className="relative pb-4">
                  <span className="absolute -left-[74px] top-0 w-11 text-right text-[12.5px] font-semibold tabular-nums">
                    {item.arriveAt}
                  </span>
                  <span
